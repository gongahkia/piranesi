...
def process_book_cover(image_path):
    ...
    try:
        ...
        contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) # contour detection
        largest_contour = max(contours, key=cv2.contourArea) # largest contour assumed to be book cover
